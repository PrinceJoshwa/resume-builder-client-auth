...
export default function Template1() {
  ...
  const downloadPDF = () => {
    const { personalInfo, skills, experience, education, certifications } = formData;
    ...
    pdf.text(personalInfo.title || 'Professional Title', margin, yPos);

    yPos += 30;
    pdf.setFontSize(10);
    pdf.setTextColor(0, 0, 0);
    if (personalInfo.email) pdf.text(`Email: ${personalInfo.email}`, margin, yPos);
    if (personalInfo.phone) pdf.text(`Phone: ${personalInfo.phone}`, margin + 200, yPos);
    if (personalInfo.location) pdf.text(`Location: ${personalInfo.location}`, margin + 400, yPos);
    ...
    if (skills.length > 0 && skills[0]) {
      ...
      const lines = addWrappedText(skillsText, margin, yPos, contentWidth);
      yPos += (lines * 12);
    }
    ...
    if (experience.length > 0 && experience[0].title) {
      ...
      experience.forEach(exp => {
        ...
        pdf.text(`${exp.startDate} - ${exp.endDate}`, margin + 200, yPos);

        yPos += 15;
        const lines = addWrappedText(exp.description, margin, yPos, contentWidth);
        yPos += (lines * 12);
      });
    }
    ...
    if (education.length > 0 && education[0].degree) {
      ...
      education.forEach(edu => {
        ...
        pdf.text(`${edu.startDate} - ${edu.endDate}`, margin + 200, yPos);
        yPos += 15;
      });
    }
    ...
    if (certifications.length > 0 && certifications[0].name) {
      ...
      certifications.forEach(cert => {
        ...
        pdf.text(cert.date, margin + 200, yPos);
        yPos += 15;
      });
    }
